...
def read_file(file_path):
    ...
    df = df[['naics_code', 'poi_cbg', 'visitor_daytime_cbgs']]
    df = df.dropna()
    return df
...
def gen_category(df, poi_category=poi_category):
    ...
    df = df[['category','poi_cbg', 'visitor_daytime_cbgs']]
    return df
...
def aggregate(df):
    ...
    df.loc[:, 'visitor_daytime_cbgs'] = df['visitor_daytime_cbgs'].str.replace(r'(?<=.)[{}](?=.)', '', regex=True) # Use regex to remove any { or } that is not at the start or end
    return df


# transform original dataframe to od dataframe
def transform_od(df):
    df.loc[:, 'visitor_daytime_cbgs'] = df['visitor_daytime_cbgs'].apply(ast.literal_eval)
    df['visitor_count'] = df['visitor_daytime_cbgs'].apply(get_dict_values)
    df['visitor_daytime_cbgs'] = df['visitor_daytime_cbgs'].apply(get_dict_keys)
    df = df.explode(['visitor_daytime_cbgs', 'visitor_count'])
    return df


# pre aggregate data to reduce file size
def pre_aggregate(df):
    df = df.groupby(['visitor_daytime_cbgs','category', 'poi_cbg']).agg({'visitor_count':'sum'})
    return df
...
def main():
    ...
    os.makedirs(f'{year}_work_preprocessed',exist_ok=True)
    file_list = [[f'raw/{x}', f'{year}_work_preprocessed/{y}'] for x,y in file_list]
